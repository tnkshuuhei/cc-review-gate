...
既定は英語です。指摘は次ターンの Claude への指示としてそのまま渡るものなので、`/gate log` を読む人が読めれば足ります。
...
スキップした理由（`no code changes in the previous turn`、`only ignoreGlobs matches were changed` など）が記録されています。

**`claude executable not found; review skipped` とログに出る**
